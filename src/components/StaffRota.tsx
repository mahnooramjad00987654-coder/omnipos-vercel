...
const StaffRota = () => {
    ...
    return (
        <div className="flex flex-col gap-8 animate-in fade-in duration-700">
            {/* Header Area */}
            <div className="glass-card rounded-[2.5rem] p-8 border border-text/10 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-primary/20 text-primary rounded-2xl flex items-center justify-center shadow-lg shadow-primary/10">
                        <CalendarDays size={28} />
                    </div>
                    <div>
                        <h2 className="text-2xl font-black text-text uppercase tracking-tight">Staffing Logic</h2>
                        <p className="text-[10px] font-bold text-muted uppercase tracking-widest">Mandatory Levels & Roster Grid</p>
                    </div>
                </div>

                <div className="flex p-1 bg-glass/40 rounded-2xl border border-text/10">
                    {['Directory', 'Payroll', 'Roles', 'Scheduling'].map(tab => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-primary text-slate-950 shadow-lg' : 'text-muted hover:text-text'}`}
                        >
                            {tab}
                        </button>
                    ))}
