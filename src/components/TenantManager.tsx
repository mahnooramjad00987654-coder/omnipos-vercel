...
const TenantManager = () => {
    const { tenants, addTenant, updateTenant, deleteTenant, currentTenantId, setTenant } = useStore();
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                {filteredTenants.map((t, i) => (
                    <motion.div
                        ...
                        className={`glass-card p-8 rounded-[2.5rem] border transition-all flex flex-col gap-6 group hover:shadow-2xl hover:shadow-primary/5 ${currentTenantId === t.id ? 'border-primary/40 bg-primary/10' : 'border-text/10 bg-glass/20 hover:border-text/20'}`}
                    >
                        <div className="flex justify-between items-start">
                            ...
                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button onClick={() => handleDelete(t.id, t.name)} className="p-2 bg-red-400/10 text-red-400 hover:bg-red-400/20 rounded-xl transition-colors"><Trash2 size={16} /></button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex justify-between items-center">
                                <div className="text-[10px] font-black text-primary uppercase tracking-widest">ID: {t.id}</div>
                        ...
                        <button
                            ...
                            className={`w-full py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${currentTenantId === t.id ? 'bg-primary text-slate-950 shadow-xl shadow-primary/20' : 'bg-glass/40 text-muted hover:text-text border border-text/10 hover:border-text/20'}`}
                        >
                            {currentTenantId === t.id ? 'Current Location' : 'Switch to Location'}
                        </button>
                    </motion.div>
                ))}
            </div>
